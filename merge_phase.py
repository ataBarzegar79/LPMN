...
def merge_algorithm(graph: Graph, nodes_additional_data, page_rank):
    for i in range(2):
        communities = get_communities(nodes_additional_data, nodes_additional_data, graph, page_rank)
...
def get_communities(nodes, nodes_additional_data, graph: Graph, page_rank):
    communities = dict()
    ...
    for community in communities:
        candidate_for_target_node_page_rank = 0
        for node in communities[community][0]:
            p_rank = page_rank[node]
            if p_rank >= candidate_for_target_node_page_rank:
                target_node = node
                candidate_for_target_node_page_rank = p_rank
